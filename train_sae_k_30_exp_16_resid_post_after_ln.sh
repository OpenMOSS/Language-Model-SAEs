...
LOGDIR="$(pwd)/logs/BT4_sae/k_30_exp_16"
mkdir -p "$LOGDIR"

for LR in 4e-5 8e-5; do
  echo "===> lr $LR"
  torchrun --nnodes=1 --master-port=$((29440+RANDOM%1000)) \
    exp/train_sae_BT4.py \
        --lr $LR \
        --layer 13 \
        --k 30 \
        --exp_factor 16 \
    > "$LOGDIR/BT4_sae_l13_lr_${LR}.log" 2>&1
done
...
for L in $(seq 0 1); do
  echo "===> layer $L"
  torchrun --nnodes=1 --master-port=$((29440+L)) \
    exp/train_sae_BT4.py \
      --lr 1e-3 \
      --layer "$L" \
      --k 30 \
      --exp_factor 16 \
    > "$LOGDIR/BT4_sae_l${L}_mid.log" 2>&1
done
