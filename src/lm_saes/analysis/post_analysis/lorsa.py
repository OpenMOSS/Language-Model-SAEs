...
class LorsaPostAnalysisProcessor(PostAnalysisProcessor):
    ...
    def _process_tensors(
        ...
        print('------------------------')
        print(interested_pairs.shape)
        print('max, min:', interested_pairs[:, 1].max(), interested_pairs[:, 1].min())
        x = torch.zeros_like(interested_pairs[:, 0], dtype=bool, device=interested_pairs.device)
        for batch_data in activation_stream:
            ...
            for i, m in enumerate(meta):
                ...
                # if m['context_idx'] > 195325:
                #     print(m['context_idx'], visited)
                #     print('unvisited:', interested_pairs[~x])

                interested_pairs_idx = (data_idx == interested_pairs).all(dim=1)
                x = x | interested_pairs_idx
