...
def get_residual_stream_components_single_module(
    ...
    single_side_activation_matrix = torch.sparse_coo_tensor(
        activation_matrix.indices()[:, feature_mask],
        activation_matrix.values()[feature_mask]
    )  # type: ignore
    decoder_vec_select_fn = (
        select_scaled_decoder_vecs_lorsa if use_lorsa 
        else select_scaled_decoder_vecs_transcoder
    )
    feature_contribution_to_res_stream = decoder_vec_select_fn(
        single_side_activation_matrix.coalesce(),
        model.lorsas if use_lorsa else model.transcoders
    )
    
    replacement_model = model.lorsas if use_lorsa else model.transcoders
    replacement_model_decoder_biases = torch.stack([
        replacement_model[i].b_D for i in range(layer_int)
    ]) if layer_int > 0 else torch.zeros(
        0,
        replacement_model[0].b_D.shape[0],
        device=error_vecs.device
    )
    
    replacement_model_error_vecs = error_vecs[: model.cfg.n_layers] if use_lorsa else error_vecs[model.cfg.n_layers :]
    replacement_model_error_vecs = replacement_model_error_vecs[:layer_int, interested_pos]  # n_layer_below_target_layer d_model
...
def probe_linear_equivalent_for_ln(components: ResStreamComponents, ln: torch.nn.Module) -> ResStreamComponents:
    ...
    assert torch.allclose(
        post_ln_components.sum(0) + b_recovered,
        ln_out,
        atol=1e-4
    )
...
def get_single_side_QK_components(
    ...
    components = probe_linear_equivalent_for_ln(
        components, model.blocks[layer_int].ln1
    )
    with torch.no_grad():
        ...
        components = components.update_components(
            components.components / (lorsa.cfg.attn_scale ** 0.5)
        )
...
def extract_QK_tracing_result(
    ...
    pair_wise_contributors = list(zip(
