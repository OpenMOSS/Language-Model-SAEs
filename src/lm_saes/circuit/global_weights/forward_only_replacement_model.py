...
class ForwardOnlyReplacementModel(ReplacementModel):
    def _get_layer_transcoder(self, layer: int):
        """
        get the transcoder for the given layer.

        compatible with two cases:
        - old version TranscoderSet / CrossLayerTranscoder style: support integer index
        - current nn.ModuleDict style: use string layer number as key
        """
        return self.transcoders[str(layer)]  # type: ignore[index]
    ...
    def setup_attribution(
        ...
        self.run_with_hooks(inputs, fwd_hooks=activation_hooks)
    ...
    def _get_activation_caching_hooks(
        ...
        def cache_activations_mlp(acts, hook, layer, zero_bos):
            ...
            transcoder_acts = self.transcoders[str(layer)].encode(
                acts, apply_activation_function=apply_activation_function
            )
    ...
    def prepare_virtual_weights(self, interested_features):
        ...
        assert not self.cfg.parallel_attn_mlp, "Parallel attn mlp (like pythia series) is not supported yet"
        assert self.cfg.normalization_type in ["LN", "RMS"], "Only LN and RMS normalization are supported"
        
        self.interested_features = interested_features
        
        # lorsa_encoders = torch.stack([
        #     self.lorsas[layer].W_V.to(torch.float32) * self.blocks[layer].ln1.w[:, None] for layer in range(self.cfg.n_layers)
        # ]).to(self.cfg.dtype)  # n_layers, d_model, d_sae
        
        lorsa_encoders = torch.stack([
            self.lorsas[layer].W_V.T.to(torch.float32) for layer in range(self.cfg.n_layers)
        ]).to(self.cfg.dtype)  # n_layers, d_model, d_sae
        
        lorsa_decoders = torch.stack([
            self.lorsas[layer].W_O for layer in range(self.cfg.n_layers)
        ...
        transcoder_encoders = torch.stack([
            self.transcoders[str(layer)].W_E.to(torch.float32) for layer in range(self.cfg.n_layers)
        ]).to(self.cfg.dtype)  # n_layers, d_model, d_sae
        
        # transcoder_decoders = self.transcoders.W_D  # n_layers, d_model, d_sae
    
        transcoder_decoders = torch.stack([
            self.transcoders[str(layer)].W_D for layer in range(self.cfg.n_layers)
        ])  # n_layers, d_sae, d_model
        ...
        self.upstream_transcoder_attribution = torch.zeros(
            batch_size,
            self.cfg.n_layers,
            self.transcoders[str(0)].cfg.d_sae,
            dtype=self.cfg.dtype,
            device=self.cfg.device
        )
    ...
    def parse_global_weight_results(self, top_k=10):
        ...
        for i in range(len(self.interested_features)):
            ...
            ) if self.interested_features.sublayer_index()[i] > 0 else BatchedFeatures.empty()
            ...
            ) if self.interested_features.sublayer_index()[i] <= self.cfg.n_layers - 1 else BatchedFeatures.empty()
    ...
    def record_activation_matrix(
        ...
        # Sparse Tensor doens't support three-dimensional indexing, so we convert to dense tensor
        # acts = []
        # for i in range(len(self.interested_features)):
        ...
        acts = []
        for i in range(len(self.interested_features)):
            layer_i = int(self.interested_features.layer[i].item())
            feat_i = int(self.interested_features.index[i].item())

            if self.interested_features.is_lorsa[i]:
                layer_acts = lorsa_activation_matrix[layer_i]  # [pos, d_sae] (sparse or dense)
            else:
                layer_acts = clt_activation_matrix[layer_i]    # [pos, d_sae] (sparse or dense)

            if layer_acts.is_sparse:
                layer_acts = layer_acts.to_dense()

            acts.append(layer_acts[:, feat_i])  # [pos]

        acts = torch.stack(acts)  # [batch, pos]
