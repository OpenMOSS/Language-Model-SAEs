...
def distributed_topk(
    ...
    *,
    ...
    # from lm_saes.utils.distributed import mesh_dim_size

    # with torch.no_grad():
    #     # local_x = torch.clamp(local_x, min=0.0)
    #     local_topk_values, _ = local_tensor.topk(k=k, dim=-1)
    #     global_topk_values = [torch.zeros_like(local_topk_values) for _ in range(mesh_dim_size(device_mesh, "model"))]

    #     torch.distributed.all_gather(global_topk_values, local_topk_values, group=device_mesh.get_group("model"))

    #     global_topk_values = torch.cat(global_topk_values, dim=-1)
    #     k = global_topk_values.shape[-1] - k + 1
    #     threshold, _ = torch.kthvalue(global_topk_values, k=k, dim=-1)
    #     while threshold.ndim < local_tensor.ndim:
    #         threshold = threshold[..., None]
