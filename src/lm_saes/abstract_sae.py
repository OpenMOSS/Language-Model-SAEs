...
class AbstractSparseAutoEncoder(HookedRootModule, ABC):
    ...
    def from_config(cls, cfg: BaseSAEConfig, device_mesh: DeviceMesh | None = None, fold_activation_scale: bool = True) -> Self:
        ...
        if "proj_bias" in state_dict.keys():
            del state_dict["proj_bias"]
        if "proj_weight" in state_dict.keys():
            del state_dict["proj_weight"]
        if "relative_sigular_value" in state_dict.keys():
            del state_dict["relative_sigular_value"]
        if "variance_factor" in state_dict.keys():
            del state_dict["variance_factor"]
