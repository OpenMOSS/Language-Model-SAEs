...
class ActivationBatchler(BaseActivationProcessor[Iterable[dict[str, Any]], Iterable[dict[str, Any]]]):
    ...
    def process(self, data: Iterable[dict[str, Any]], **kwargs) -> Iterable[dict[str, torch.Tensor]]:
        ...
        print(f'{dp_size = }')
        for d in data:
            # Drop metadata before batching to avoid mismatched batch sizes across DP shards
            d.pop("meta", None)

            def get_batch_size(x):
                # For DTensor, len(x) returns local size, so multiply by dp_size to get global size
                # For lists/tensors, len(x) is local size on this rank, so also multiply by dp_size
                # return len(x) if isinstance(x, DTensor) else len(x) * dp_size
                return len(x) * dp_size if (isinstance(x, DTensor) or dp_size > 1) else len(x)
