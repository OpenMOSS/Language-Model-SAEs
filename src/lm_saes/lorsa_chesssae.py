...
class SmolGenLorsa(nn.Module):
    ...
    def __init__(self, d_model: int, n_qk_heads: int, d_qk_head: int, n_ctx: int, device_mesh: Optional[DeviceMesh] = None, dim_maps: Optional[dict[str, DimMap]] = None, dtype: Optional[torch.dtype] = None):
        super().__init__()
        ...
        if device_mesh is None:
            ...
            self.dense2 = nn.Linear(256, n_qk_heads * 256)
            self.ln2 = nn.LayerNorm(n_qk_heads * 256)
            self.smol_weight_gen = nn.Linear(256, n_ctx * n_ctx, bias=False)
        else:
            # Distributed parameters
            assert dim_maps is not None, "dim_maps must be provided when device_mesh is not None"
            if dtype is None:
                dtype = torch.get_default_dtype()
            
            # Compress: [d_model, 32] - shard along d_model dimension (dim 0)
            self.compress = nn.Linear(d_model, 32, bias=False)
            ...
            # Dense1: [compressed_size, 256] - replicate (shared across heads)
            self.dense1 = nn.Linear(compressed_size, 256)
            ...
            # Dense2: [256, n_qk_heads * 256] - shard along n_qk_heads dimension (dim 1)
            self.dense2 = nn.Linear(256, n_qk_heads * 256)
            ...
            # LN2: [n_qk_heads * 256] - shard along n_qk_heads dimension
            self.ln2 = nn.LayerNorm(n_qk_heads * 256)
            ...
            # SmolWeightGen: [256, n_ctx * n_ctx] - shard along first dimension (dim 0) but replicate output
            # Note: This is tricky because smol_weight_gen operates per-head but outputs shared weights
            # We'll shard along the input dimension (256) but replicate the output
            self.smol_weight_gen = nn.Linear(256, n_ctx * n_ctx, bias=False)
    ...
    def forward(self, x: torch.Tensor | DTensor) -> torch.Tensor | DTensor:
        """
        Args:
            x: [batch_size, n_ctx, d_model] (can be DTensor)
        Returns:
            weights: [batch_size, n_qk_heads, n_ctx, n_ctx] (can be DTensor)
        """
        # Handle DTensor input
        is_dtensor = isinstance(x, DTensor)
        if is_dtensor and self._dim_maps is not None:
            # Redistribute input to match compress layer's expected placement
            # compress.weight expects input sharded along d_model (dim 2)
            if "smolgen.compress.weight" in self._dim_maps:
        ...
        # Redistribute for dense1 if needed
        if is_dtensor and self._dim_maps is not None and "smolgen.dense1.weight" in self._dim_maps:
            assert isinstance(x_flat, DTensor), "x_flat should be DTensor when is_dtensor is True"
        ...
        # Redistribute for dense2 if needed
        if is_dtensor and self._dim_maps is not None and "smolgen.dense2.weight" in self._dim_maps:
            assert isinstance(x, DTensor), "x should be DTensor when is_dtensor is True"
        ...
        # Redistribute for smol_weight_gen if needed
        if is_dtensor and self._dim_maps is not None and "smolgen.smol_weight_gen.weight" in self._dim_maps:
            # Reshape back to [batch_size * n_qk_heads, 256] for linear layer
            x_reshaped = x.view(batch_size * self.n_qk_heads, 256)
            assert isinstance(x_reshaped, DTensor), "x_reshaped should be DTensor when is_dtensor is True"
            x_reshaped = self._dim_maps["smolgen.smol_weight_gen.weight"].redistribute(x_reshaped)
            # Apply smol_weight_gen and reshape back
            weights_flat = self.smol_weight_gen(x_reshaped)  # [batch_size * n_qk_heads, n_ctx * n_ctx]
            weights = weights_flat.view(batch_size, self.n_qk_heads, self.n_ctx, self.n_ctx)
        else:
            # Generate attention weights for each head
            weights_flat = self.smol_weight_gen(x.view(batch_size * self.n_qk_heads, 256))  # [batch_size * n_qk_heads, n_ctx * n_ctx]
            weights = weights_flat.view(batch_size, self.n_qk_heads, self.n_ctx, self.n_ctx)
...
class LowRankSparseAttention(AbstractSparseAutoEncoder):
    ...
    def init_W_D_with_active_subspace_per_head(self, activation_batch: dict[str, torch.Tensor], encoder_layer):
        ...
        if self.device_mesh is not None:
            ...
            model_parallel_rank = self.device_mesh.get_local_rank(mesh_dim="model")
            model_parallel_size = mesh_dim_size(self.device_mesh, "model")
            orig_start_idx = model_parallel_rank * orig_n_heads // model_parallel_size
            orig_end_idx = orig_start_idx + orig_n_heads // model_parallel_size
        ...
        **kwargs
        ...
        if getattr(self.cfg, "use_smolgen", True):
            if hasattr(self, "smolgen"):
                ...
                # Convert attn_mask to local tensor for scaled_dot_product_attention
                # scaled_dot_product_attention may not properly handle DTensor attn_mask parameter
                # Converting to local is safe because mask can be replicated on all ranks
        ...
        if hasattr(self, 'attn_scale'):
            if isinstance(self.attn_scale, DTensor):
                scale = 1 / float(self.attn_scale.to_local().item())
            else:
                scale = 1 / float(self.attn_scale.item())
        else:
            assert self.cfg.attn_scale is not None
        ...
        **kwargs,
        ...
        if sparsity_loss_type is not None:
            if sparsity_loss_type == "power":
                ...
                raise ValueError(f"sparsity_loss_type {sparsity_loss_type} not supported.")
    ...
    def dim_maps(self) -> dict[str, DimMap]:
        ...
        # SmolGenLorsa parameter mappings
        if self.cfg.use_smolgen:
            # compress: [32, d_model] - shard along d_model dimension (dim 1)
            lorsa_maps["smolgen.compress.weight"] = DimMap({"model": 1})
            # dense1: [256, compressed_size] - replicate (shared across heads)
            lorsa_maps["smolgen.dense1.weight"] = DimMap({})
            lorsa_maps["smolgen.dense1.bias"] = DimMap({})
            # ln1: [256] - replicate
            lorsa_maps["smolgen.ln1.weight"] = DimMap({})
            lorsa_maps["smolgen.ln1.bias"] = DimMap({})
            # dense2: [n_qk_heads * 256, 256] - shard along n_qk_heads dimension (dim 0)
            lorsa_maps["smolgen.dense2.weight"] = DimMap({"model": 0})
            lorsa_maps["smolgen.dense2.bias"] = DimMap({"model": 0})
            # ln2: [n_qk_heads * 256] - shard along n_qk_heads dimension (dim 0)
            lorsa_maps["smolgen.ln2.weight"] = DimMap({"model": 0})
            lorsa_maps["smolgen.ln2.bias"] = DimMap({"model": 0})
            # smol_weight_gen: [n_ctx * n_ctx, 256] - shard along input dimension (dim 1) but replicate output
            # Actually, we'll replicate it since it's shared across heads
            lorsa_maps["smolgen.smol_weight_gen.weight"] = DimMap({})
