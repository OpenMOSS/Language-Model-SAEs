...
class CNNSparseAutoEncoder(AbstractSparseAutoEncoder):
    def __init__(self, cfg: CNNSAEConfig, device_mesh: DeviceMesh | None = None):
        ...
        self.W_conv = nn.Parameter(torch.empty(cfg.d_model, 1, 7, 7, device=cfg.device, dtype=cfg.dtype))
        self.b_conv = nn.Parameter(torch.zeros(cfg.d_model, device=cfg.device, dtype=cfg.dtype))
    ...
    def encode(
        ...
        **kwargs,
        ...
        hidden_pre = F.conv2d(x, self.W_conv, self.b_conv, padding=3, groups=self.cfg.d_model)
    ...
    def init_parameters(self, **kwargs):
        ...
        W_conv = torch.empty(self.cfg.d_model, 1, 7, 7, device=self.cfg.device, dtype=self.cfg.dtype).uniform_(-(self.cfg.d_model)**(-0.5), (self.cfg.d_model)**(-0.5))
