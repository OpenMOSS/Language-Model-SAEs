...
class LayerNorm(nn.Module):
    def __init__(self, d_model: int, eps: float = 1e-3) -> None:
        super().__init__()
        self.eps = float(eps)
...
class AttentionBody(nn.Module):
    def __init__(self, d_model: int = 1024) -> None:
        ...
        self.main_linear = nn.Linear(624, self.d_model)
        self.ma_gating_mul = nn.Parameter(torch.randn(64, self.d_model))
        self.ma_gating_add = nn.Parameter(torch.randn(64, self.d_model))
        self.ffn_dense1 = nn.Linear(self.d_model, 1536)
        self.ffn_dense2 = nn.Linear(1536, self.d_model)
        self.ffn_alpha = nn.Parameter(torch.ones(1))
        self.ln = nn.LayerNorm(self.d_model, eps=1e-3)
        self.ln2 = nn.LayerNorm(self.d_model, eps=1e-3)
...
class SmolGen(nn.Module):
    def __init__(self, d_model: int = 1024, n_heads: int = 32) -> None:
        super().__init__()
        self.n_heads = int(n_heads)
        self.compress = nn.Linear(d_model, 32, bias=False)
        self.dense1 = nn.Linear(2048, 256)
        self.ln1 = nn.LayerNorm(256, eps=1e-3)
        self.dense2 = nn.Linear(256, 256 * self.n_heads)
        self.ln2 = nn.LayerNorm(256 * self.n_heads, eps=1e-3)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        attn_out = (
            attn_out.permute(0, 2, 1, 3).contiguous().view(batch_size, seq_len, self.d_model)
        )
...
class LC0MLP(nn.Module):
    def __init__(self, d_model: int, d_ff: int, activation: str = "mish") -> None:
        super().__init__()
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model: int = 1024, n_heads: int = 32, d_ff: int = 1536) -> None:
        super().__init__()
        self.n_heads = int(n_heads)
        self.d_model = int(d_model)
        self.mha = MultiHeadAttention(self.d_model, self.n_heads)
        self.ln1 = LayerNorm(self.d_model, eps=1e-3)
        self.ln2 = LayerNorm(self.d_model, eps=1e-3)
        self.mlp = LC0MLP(self.d_model, int(d_ff))
...
class PolicyHead(nn.Module):
    def __init__(self, d_model: int = 1024, policy_dim: int = 1858) -> None:
        super().__init__()
...
class ValueHead(nn.Module):
    def __init__(self, d_model: int = 1024, d_value_head: int = 128) -> None:
        super().__init__()
        self.d_model = int(d_model)
        self.embed = nn.Linear(self.d_model, d_value_head)
...
class MLHHead(nn.Module):
    def __init__(self, d_model: int = 1024, d_mlh_head: int = 32) -> None:
        super().__init__()
        self.d_model = int(d_model)
        self.embed = nn.Linear(self.d_model, d_mlh_head)
...
class CleanLC0Model(nn.Module):
    def __init__(
        ...
        self.policy_head = PolicyHead(self.d_model)
...
@torch.no_grad()
def convert_bt4_onnx_to_clean_state_dict(
    *,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Convert BT4 ONNX weights to CleanLC0Model state_dict.")
    p.add_argument("--onnx", required=True, help="Path to BT4 ONNX file.")
    p.add_argument("--out", required=True, help="Output path for torch state_dict (.pt).")
...
def main() -> None:
    ...
    if device.startswith("cuda") and not torch.cuda.is_available():
        device = "cpu"
    out_path = convert_bt4_onnx_to_clean_state_dict(
