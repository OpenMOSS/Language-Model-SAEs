...
def analyze_node_activation_impact(
    ...
    target_nodes: Union[List[Union[str, dict, "Node", Tuple[int, int, int, str]]], Union[str, dict, "Node", Tuple[int, int, int, str]]],
    ...
    """
    Analyze how steering source nodes affects target nodes' activations.
    
    This function implements a causal intervention: it steers the activation of source nodes
    and measures how this affects the activation of target nodes, revealing feature
    ...
    Args:
        ...
        target_nodes: Target node(s) to measure activation change for. Can be a single node
                     (Node object, string, dict, or tuple) or a list of nodes. All target nodes
                     must be at higher layers than all steering nodes.
    ...
    Returns:
        ...
        - target_nodes: List of dictionaries, each containing:
    ...
    # Handle single target node or list of target nodes
    if isinstance(target_nodes, list):
        target_node_list = _normalize_nodes(target_nodes)
    else:
        # Single target node - convert to list for uniform processing
        target_node_list = _normalize_nodes([target_nodes])
    # ======================
    # Validate causal ordering (pairwise)
    # ======================
    for s_node in steering_node_list:
        for t_node in target_node_list:
            if s_node.layer < t_node.layer:
                continue
            if (s_node.layer == t_node.layer and s_node.feature_type == "lorsa" and t_node.feature_type == "transcoder"):
                continue
            raise ValueError(
                f"Invalid causal ordering between nodes:\n"
                f"  Steering node: layer={s_node.layer}, type={s_node.feature_type}\n"
    ...
    def get_activation_value(node: Node, activations_dict: Dict[str, List[torch.Tensor]]) -> float:
        ...
        # Coalesce sparse tensor to merge duplicate indices
        activations = activations.coalesce()
        
        # Find activation at (batch=0, pos, feature)
        # indices shape: [3, nnz] where [0, :] is batch, [1, :] is pos, [2, :] is feature
        indices = activations.indices()  # [3, nnz]
        values = activations.values()  # [nnz]
        
        # Check if batch=0, pos matches, and feature matches
        batch_match = indices[0] == 0
        pos_match = indices[1] == pos
        feature_match = indices[2] == feature
        
        # All three conditions must be true
        matches = batch_match & pos_match & feature_match
        
        if matches.any():
            # Get the first matching value (should be only one after coalesce)
            return values[matches][0].item()
        else:
            print(f"No activation found for node: {node}")
            return 0.0
    ...
    # Get original activations of all target nodes
    # Use node string representation as key since Node objects are not hashable
    target_original_activations = {
        repr(node): get_activation_value(node, activations_dict)
    ...
    # Get modified activations for all target nodes and calculate metrics
    target_results = []
    for target_node_obj in target_node_list:
        node_key = repr(target_node_obj)
        original_activation = target_original_activations[node_key]
...
def analyze_node_interaction(
    ...
    """
    Analyze steering impact on one or more target nodes.
    ...
    Returns:
        Analysis result containing information for all target nodes. The result dictionary includes:
        - steering_scale: Steering scale used
        - steering_nodes_count: Number of steering nodes applied
        - steering_details: Details about each steering node application
        - target_nodes: List of dictionaries, each containing results for one target node:
            - target_node: String representation of target node
