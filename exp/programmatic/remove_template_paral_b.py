...
def filter_and_pad_tensor_forb(tokens, activation, otemplate, filename):
    original_length = len(tokens)
    tokens_list = tokens.tolist()
    activation_list = activation.tolist()
    o_list = otemplate.tolist()
    ...
    # 添加断言检查 token 是否对齐                                  ######################
    try:
        assert filtered_tokens_list == o_list, f"tokens not aligned: {filename}"
...
def main(
    input_folder="/inspire/hdd/global_user/hezhengfu-240208120186/jiaxing_activations/reasondata-b-2d-801M/blocks.15.hook_resid_post",
    output_folder="/inspire/hdd/global_user/hezhengfu-240208120186/jiaxing_activations/reasondata-b-2d-801M-f/blocks.15.hook_resid_post",
    ...
    with Pool(cpu_cores) as pool:
        results = pool.map(process_file, [(input_folder, output_folder, template_folder, f, "b") for f in all_files])
