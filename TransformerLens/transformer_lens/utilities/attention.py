...
def simple_attn_linear(
    ...
    b_ = einops.rearrange(b, "head_index d_head -> (head_index d_head)") if b.ne(0).any() else None
    return F.linear(input, w, b_).view(input.shape[0], input.shape[1], b.shape[0], b.shape[1])
