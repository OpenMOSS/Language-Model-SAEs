...
def convert_llama_weights(llama, cfg: HookedTransformerConfig):
    ...
    for l in range(cfg.n_layers):
        ...
        # parameters should stay as bitsandbytes.nn.modules.Params4bit
        if not cfg.load_in_4bit:
            state_dict[f"blocks.{l}.mlp.W_in.weight"] = llama.model.layers[l].mlp.up_proj.weight
            state_dict[f"blocks.{l}.mlp.W_gate.weight"] = llama.model.layers[l].mlp.gate_proj.weight
            state_dict[f"blocks.{l}.mlp.W_out.weight"] = llama.model.layers[l].mlp.down_proj.weight
        else:
