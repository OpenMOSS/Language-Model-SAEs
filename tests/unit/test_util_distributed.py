...
from lm_saes.utils.distributed import mesh_dim_size, mesh_rank
...
def test_get_mesh_dim_size_and_rank_2d_distributed():
    ...
    # Test mesh_dim_size for both dimensions
    a_dim_size = mesh_dim_size(device_mesh, "a")
    b_dim_size = mesh_dim_size(device_mesh, "b")
    c_dim_size = mesh_dim_size(device_mesh, "c")
    ...
    # Test mesh_rank
    current_mesh_rank = mesh_rank(device_mesh)
    assert current_mesh_rank == rank, f"Expected mesh rank {rank}, got {current_mesh_rank}"

    sub_mesh = device_mesh["b", "c"]
    sub_mesh_rank = mesh_rank(sub_mesh)
    assert sub_mesh_rank == rank % 4, f"Expected sub mesh rank {rank % 4}, got {sub_mesh_rank}"
    b_dim_size = mesh_dim_size(sub_mesh, "b")
    assert b_dim_size == 2, f"Expected b dimension size 2, got {b_dim_size}"
    c_dim_size = mesh_dim_size(sub_mesh, "c")
    assert c_dim_size == 2, f"Expected c dimension size 2, got {c_dim_size}"
    ...
    a_dim_size = mesh_dim_size(sub_mesh, "a")
    assert a_dim_size == 2, f"Expected a dimension size 2, got {a_dim_size}"
    c_dim_size = mesh_dim_size(sub_mesh, "c")
    assert c_dim_size == 2, f"Expected c dimension size 2, got {c_dim_size}"
