...
if __name__ == "__main__":
    ...
    parser.add_argument("--layer-ratio", type=float, default=0.5)
    parser.add_argument("--layer", type=int, default=None)
    parser.add_argument("--model-batch-size", type=int, default=32)
    args = parser.parse_args()

    if args.layer is None:
        layer = int(args.layer_ratio * n_layers_map[args.size])
    else:
        layer = args.layer
    ...
    for step in steps[args.start_step : args.end_step]:
        print(f"Generating activations for pythia-{args.size} at step {step} at layer {layer}")
        settings = GenerateActivationsSettings(
            ...
            output_dir=os.path.expanduser(
                f"~/activations/SlimPajama-3B-activations-pythia-{args.size}-2d-all-{dtype_suffix}/step{step}"
            ),
