...
$$
\begin{aligned}
A^h &= \operatorname{softmax}\!\left(\frac{(X W_q^h) (X W_k^h)^\top}{\sqrt{d_{\text{qk}}}}\right) \in \mathbb{R}^{n_{\text{ctx}} \times n_{\text{ctx}}} \\
\tilde{\mathbf{z}}^i &= A^h X \mathbf{w}_v^i \in \mathbb{R}^{n_{\text{ctx}} \times 1}
\end{aligned}
$$
