...
- Avoid hardcoding `DTensor` placements. Use [DimMap](https://github.com/OpenMOSS/Llamascopium/blob/main/src/llamascopium/utils/distributed/dimmap.py) (which is designed to be similar to [PartitionSpec](https://docs.jax.dev/en/latest/jax.sharding.html#jax.sharding.PartitionSpec) in JAX) to dynamically generate placements based on current `DeviceMesh`. This allows absence of some specific dimensions in `DeviceMesh`.
