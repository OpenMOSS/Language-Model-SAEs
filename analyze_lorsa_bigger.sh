CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=29441 \
  exp/analyze_lc0_lorsa_BT4.py --layer 10


cd /inspire/hdd/global_user/hezhengfu-240208120186/rlin_projects/rlin_projects/chess-SAEs-N
. .venv/bin/activate 
export PYTHONPATH=/inspire/hdd/global_user/hezhengfu-240208120186/rlin_projects/rlin_projects/chess-SAEs-N/src:$PYTHONPATH  
LOGDIR="$(pwd)/logs/analyze"
mkdir -p "$LOGDIR"

for L in $(seq 0 1); do
  echo "===> layer $L"
  CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=$((29440+L)) \
    exp/analyze_lc0_lorsa_BT4.py --layer "$L" \
    > "$LOGDIR/lorsa_BT4_bigger${L}.log" 2>&1
done


cd /inspire/hdd/global_user/hezhengfu-240208120186/rlin_projects/rlin_projects/chess-SAEs-N
. .venv/bin/activate  
export PYTHONPATH=/inspire/hdd/global_user/hezhengfu-240208120186/rlin_projects/rlin_projects/chess-SAEs-N/src:$PYTHONPATH 
LOGDIR="$(pwd)/logs/analyze"
mkdir -p "$LOGDIR"

for L in $(seq 2 3); do
  echo "===> layer $L"
  CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=$((29440+L)) \
    exp/analyze_lc0_lorsa_BT4.py --layer "$L" \
    > "$LOGDIR/lorsa_BT4_bigger${L}.log" 2>&1
done

cd /inspire/hdd/global_user/hezhengfu-240208120186/rlin_projects/rlin_projects/chess-SAEs-N
. .venv/bin/activate   
LOGDIR="$(pwd)/logs/analyze"
mkdir -p "$LOGDIR"

for L in $(seq 4 5); do
  echo "===> layer $L"
  CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=$((29440+L)) \
    exp/analyze_lc0_lorsa_BT4.py --layer "$L" \
    > "$LOGDIR/lorsa_BT4_bigger${L}.log" 2>&1
done

cd /inspire/hdd/global_user/hezhengfu-240208120186/rlin_projects/rlin_projects/chess-SAEs-N
. .venv/bin/activate   
LOGDIR="$(pwd)/logs/analyze"
mkdir -p "$LOGDIR"

for L in $(seq 6 7); do
  echo "===> layer $L"
  CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=$((29440+L)) \
    exp/analyze_lc0_lorsa_BT4.py --layer "$L" \
    > "$LOGDIR/lorsa_BT4_bigger${L}.log" 2>&1
done

cd /inspire/hdd/global_user/hezhengfu-240208120186/rlin_projects/rlin_projects/chess-SAEs-N
. .venv/bin/activate   
LOGDIR="$(pwd)/logs/analyze"
mkdir -p "$LOGDIR"

for L in $(seq 8 9); do
  echo "===> layer $L"
  CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=$((29440+L)) \
    exp/analyze_lc0_lorsa_BT4.py --layer "$L" \
    > "$LOGDIR/lorsa_BT4_bigger${L}.log" 2>&1
done

cd /inspire/hdd/global_user/hezhengfu-240208120186/rlin_projects/rlin_projects/chess-SAEs-N
. .venv/bin/activate   
LOGDIR="$(pwd)/logs/analyze"
mkdir -p "$LOGDIR"

for L in $(seq 10 11); do
  echo "===> layer $L"
  CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=$((29440+L)) \
    exp/analyze_lc0_lorsa_BT4.py --layer "$L" \
    > "$LOGDIR/lorsa_BT4_bigger${L}.log" 2>&1
done

cd /inspire/hdd/global_user/hezhengfu-240208120186/rlin_projects/rlin_projects/chess-SAEs-N
. .venv/bin/activate   
LOGDIR="$(pwd)/logs/analyze"
mkdir -p "$LOGDIR"

for L in $(seq 12 13); do
  echo "===> layer $L"
  CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=$((29440+L)) \
    exp/analyze_lc0_lorsa_BT4.py --layer "$L" \
    > "$LOGDIR/lorsa_BT4_bigger${L}.log" 2>&1
done

cd /inspire/hdd/global_user/hezhengfu-240208120186/rlin_projects/rlin_projects/chess-SAEs-N
. .venv/bin/activate   
LOGDIR="$(pwd)/logs/analyze"
mkdir -p "$LOGDIR"

for L in 14; do
  echo "===> layer $L"
  CUDA_VISIBLE_DEVICES=0 torchrun --nproc_per_node=1 --master_port=$((29440+L)) \
    exp/analyze_lc0_lorsa_BT4.py --layer "$L" \
    > "$LOGDIR/lorsa_BT4_bigger${L}.log" 2>&1
done